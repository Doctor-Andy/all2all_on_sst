...
#include "all2all.h" // -- ADC

#include <algorithm>
#include <stdlib.h>

// Cribbed from all2all, retaining most of the code.  I've added the requirement
// that dimensions = 1. -- ADC

using namespace SST::Merlin;


topo_all2all::topo_all2all(Component* comp, Params& params) :
    Topology(comp)
{
    ...
    std::string shape;
    shape = params.find<std::string>("all2all:shape");
    if ( !shape.compare("") ) {
    }
    ...
    if (dimensions != 1 ) { // added by ADC
        output.fatal (CALL_INFO, -1, "Dimensions must = 1 for all2all.\n");
    }
    ...
    std::string width = params.find<std::string>("all2all:width", "");
    if ( width.compare("") == 0 ) {
    ...
    num_local_ports = params.find<int>("all2all:local_ports", 1);
...
void
topo_all2all::route(int port, int vc, internal_router_event* ev)
{
    int dest_router = get_dest_router(ev->getDest());
    if ( dest_router == router_id ) {
        ev->setNextPort(get_dest_local_port(ev->getDest()));
    } else {
        topo_all2all_event *tt_ev = static_cast<topo_all2all_event*>(ev);
...
topo_all2all::process_input(RtrEvent* ev)
{
    topo_all2all_event* tt_ev = new topo_all2all_event(dimensions);
    tt_ev->setEncapsulatedEvent(ev);
    tt_ev->setVC(ev->request->vn * 2);

    // Need to figure out what the all2all address is for easier
    // routing.
...
void topo_all2all::routeInitData(int port, internal_router_event* ev, std::vector<int> &outPorts)
{
    topo_all2all_init_event *tt_ev = static_cast<topo_all2all_init_event*>(ev);
    if ( tt_ev->phase == 0 ) {
...
internal_router_event* topo_all2all::process_InitData_input(RtrEvent* ev)
{
    topo_all2all_init_event* tt_ev = new topo_all2all_init_event(dimensions);
    tt_ev->setEncapsulatedEvent(ev);
    tt_ev->setVC(ev->request->vn * 2);
...
Topology::PortState
topo_all2all::getPortState(int port) const
{
...
void
topo_all2all::idToLocation(int run_id, int *location) const
{
...
void
topo_all2all::parseDimString(const std::string &shape, int *output) const
{
    size_t start = 0;
    size_t end = 0;
    for ( int i = 0; i < dimensions; i++ ) {
...
int
topo_all2all::get_dest_router(int dest_id) const
{
    return dest_id / num_local_ports;
}

int
topo_all2all::get_dest_local_port(int dest_id) const
{
...
int
topo_all2all::choose_multipath(int start_port, int num_ports, int dest_dist)
{
    if ( num_ports == 1 ) {
        return start_port;
    } else {
...
int
topo_all2all::computeNumVCs(int vns)
{
    return 2*vns;
}

int
topo_all2all::getEndpointID(int port)
{
