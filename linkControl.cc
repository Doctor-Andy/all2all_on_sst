...
void LinkControl::init(unsigned int phase)
{
    ...
    std::cout << "Inside LinkControl::init with phase = " << phase << std::endl; // added by ADC
    switch ( phase ) {
    case 0:
        {
    ...
    case 1:
        ...
        if (init_ev == NULL) {
            std::cout << "NULL ptr as init_ev.  The next line of code will fail." << std::endl; // ADC
        }
